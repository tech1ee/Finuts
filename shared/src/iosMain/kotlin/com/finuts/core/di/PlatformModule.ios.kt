...
actual val platformModule: Module = module {
    ...
    // PDF/OCR services (no Context needed on iOS)
    single { PdfTextExtractor() }
    single { OcrService() }

    // Model downloader (no Context needed on iOS)
    single<ModelDownloaderOperations> { ModelDownloader() }

    // On-device LLM inference engine (Tier 1.5)
    // Note: Requires LLM.swift setup in Xcode. See GGUFInferenceEngine docs.
    single<InferenceEngine> { GGUFInferenceEngine() }
}
