...
/**
 * Top bar for account detail screen with navigation, edit, and menu.
 */
@Composable
    ...
    Row(
        ...
        Box {
            ...
            DropdownMenu(expanded = showMenu, onDismissRequest = onDismissMenu) {
                DropdownMenuItem(
                    text = { Text("Archive", style = FinutsTypography.bodyMedium) },
                    onClick = onArchiveClick,
                    leadingIcon = { Icon(Icons.Default.Archive, null, tint = FinutsColors.TextSecondary) }
                )
                DropdownMenuItem(
                    text = { Text("Delete", style = FinutsTypography.bodyMedium, color = FinutsColors.Expense) },
